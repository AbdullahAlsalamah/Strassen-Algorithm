...
public class testValue {
	public static void main(String[] args) throws Exception {
		...
		while (!Case.equals("") && Case != null) {
			...
			int n = (int) Math.pow(2, Math.ceil(Math.log(input) / Math.log(2)));
			int b = kb.nextInt();
			String fileNameIn = kb.next();
			String fileNameOut = kb.next();

			fillInputFile(n, input, fileNameIn);
			...
			for (int i = 0; i < n; i++) {
				for (int j = 0; j < n; j++) {
					A[i][j] = fileIn.nextInt();
				}
			}

			for (int i = 0; i < n; i++) {
				for (int j = 0; j < n; j++) {
					B[i][j] = fileIn.nextInt();
				}
			}
			...
			classical = classical(A, B, n);
			strassenBase1 = Strassen(A, B, n, 1);
			strassen = Strassen(A, B, n, b);
			...
			printM(classical, "ClassicalOutput.txt");
			printM(strassenBase1, "StrasseneBase1Output.txt");
			printM(strassen, "StrassenOutput.txt");

		//	printM(C, fileNameOut);
